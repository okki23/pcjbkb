...
class video_front extends MY_Controller {
	public function __construct(){
		parent ::__construct();

		$this->load->model("m_video_front","mh");
		$this->load->model("home/m_home","mh");
		
	}
	public function index()
	{
		$data['content'] = 'video_front/view_video_front'; 
		$jumlah_data = $this->mh->jumlah_data();
		$data['con_video_front'] = $this->db->query("select * from page_video")->result();
		$data['slide'] = $this->db->query("select * from slider ORDER BY order_slide ASC")->result();
		$this->load->library('pagination');	

		$config['base_url'] = base_url().'video_front/index/';
